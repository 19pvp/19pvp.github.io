...
const dbcSchema = {
  ...
  BankBagSlotPrices: { ID: 'int', Cost: 'int' },
  BarberShopStyle: {
    ID: 'int',
    Type: 'int',
  ...
  CharBaseInfo: { ID: 'byte', RaceID: 'byte' },
  CharStartOutfit: {
    ID: 'int',
    RaceID: 'byte',
    ClassID: 'byte',
  ...
  DurabilityQuality: { ID: 'int', Data: 'float' },
  Emotes: {
    ID: 'int',
  ...
  GlyphSlot: { ID: 'int', Type: 'int', Tooltip: 'int' },
  // gtBarberShopCostBase: { Data: 'float' },
  // gtChanceToMeleeCrit: { Data: 'float' },
  // gtChanceToMeleeCritBase: { Data: 'float' },
  // gtChanceToSpellCrit: { Data: 'float' },
  // gtChanceToSpellCritBase: { Data: 'float' },
  // gtCombatRatings: { Data: 'float' },
  // gtNPCManaCostScaler: { Data: 'float' },
  // gtOCTClassCombatRatingScalar: { ID: 'uint', Data: 'float' },
  // gtOCTRegenHP: { Data: 'float' },
  // gtRegenHPPerSpt: { Data: 'float' },
  // gtRegenMPPerSpt: { Data: 'float' },
  Holidays: {
    ID: 'int',
    Duration_1: 'int',
    Duration_2: 'int',
  ...
  Movie: { ID: 'int', Filename: 'string', Volume: 'int' },
  OverrideSpellData: {
    ID: 'int',
    Spells_1: 'int',
    Spells_2: 'int',
  ...
  SpellCategory: { ID: 'int', Flags: 'int' },
  SpellDifficulty: {
    ID: 'int',
  ...
  StableSlotPrices: { ID: 'int', Cost: 'int' },
  SummonProperties: {
    ID: 'int',
    Control: 'int',
    Faction: 'int',
  ...
  TeamContributionPoints: { ID: 'int', Data: 'float' },
  TotemCategory: {
    ID: 'int',
    Name_Lang_enUS: 'string',
    Name_Lang_enGB: 'string',
...
} as const satisfies Record<string, DBCSchema>

type DBCSchemas = typeof dbcSchema
type DBCName = keyof DBCSchemas
type DBCFieldValue<T extends DBCFieldType> = T extends 'string' ? string
  : number
type DBCValue<T extends DBCName> = {
  [K in keyof DBCSchemas[T]]: DBCFieldValue<
    Extract<DBCSchemas[T][K], DBCFieldType>
  >
}
type DBCRows<T extends DBCName> = Map<number, DBCValue<T>>
type DBCRowDraft<T extends DBCName> = Partial<
  Record<keyof DBCValue<T>, string | number>
>
type SchemaEntry<T extends DBCName> = {
  [K in keyof DBCSchemas[T]]: [K, DBCSchemas[T][K]]
}[keyof DBCSchemas[T]]

const cache = new Map<DBCName, DBCRows<DBCName>>()
export const openDBC = <T extends DBCName>(name: T) => {
  const cached = cache.get(name) as DBCRows<T> | undefined
  if (cached) return cached
  const schema = dbcSchema[name]
  if (!schema) throw Error(`${name} is not a known dbc, missing schema definition`)
  const file = Deno.readFileSync(`${import.meta.dirname}/dbc/${name}.dbc`)
  const buffer = file.buffer.slice(file.byteOffset, file.byteOffset + file.byteLength)
  type DBCRow = DBCValue<T>
  ...
  const fields = Object.entries(schema) as SchemaEntry<T>[]
  ...
  const getStringAt = (start: number) => {
    let end = start - 1
    while (strings[++end] !== '\0') {
      // Scan until the null terminator in the DBC string block.
    }
    return strings.slice(start, end)
  }

  let rowIndex = -1
  const byIds: DBCRows<T> = new Map()
  while (++rowIndex < ROW_COUNT) {
    const row: DBCRowDraft<T> = {}
    ...
    while (++i < FIELD_COUNT) {
      ...
      const key = field[0] as keyof DBCValue<T>
      let value: string | number | undefined
      switch (field[1]) {
        case 'string': {
          const offset = view.getUint32(cursor, true)
          value = getStringAt(offset)
          cursor += 4
          break
        }
        case 'byte': {
      ...
      if (value === undefined) throw Error(`invalid field type ${field[1]}`)
      row[key] = value
    ...
    if (rowId === undefined) throw Error(`missing ID field for ${name}`)
    byIds.set(rowId, row as DBCRow)
  }
