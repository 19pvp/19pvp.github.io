...
import addrToIPPort from 'npm:addr-to-ip-port'
import BitField from 'npm:bitfield'
import CacheChunkStore from 'npm:cache-chunk-store'
import { chunkStoreWrite } from 'npm:chunk-store-iterator'
import cpus from 'npm:cpus'
import debugFactory from 'npm:debug'
import Discovery from '../../torrent-discovery/index.js'
import FSChunkStore from 'npm:fs-chunk-store' // browser: `fsa-chunk-store`
import ImmediateChunkStore from 'npm:immediate-chunk-store'
import ltDontHave from 'npm:lt_donthave'
import MemoryChunkStore from 'npm:memory-chunk-store'
...
import randomIterate from 'npm:random-iterate'
import { hash } from 'npm:uint8-util'
import throughput from 'npm:throughput'
import utMetadata from 'npm:ut_metadata'
import utPex from 'npm:ut_pex' // browser exclude
...
export default class Torrent extends EventEmitter {
  ...
  get downloaded () {
    ...
    for (let index = 0, len = this.pieces.length; index < len; ++index) {
      if (this.bitfield.get(index) || !this.pieces[index]) { // verified data
