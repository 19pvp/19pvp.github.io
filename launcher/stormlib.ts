...
export type StormModuleOptions = {
  /** URL or path to the generated stormlib.mjs file. */
  module: string | URL;
  /** URL or path to the generated stormlib.wasm file. */
  wasm?: string | URL;
};

export async function loadStormLib(
  options: StormModuleOptions,
): Promise<StormArchiveModule> {
  const moduleUrl = new URL(options.module, import.meta.url);
  const wasmUrl = new URL(options.wasm ?? "./stormlib.wasm", moduleUrl);
  const factory = (await import(moduleUrl.href)).default as StormModuleFactory;
  const wasmBinary = await Deno.readFile(wasmUrl);
  return new StormArchiveModule(await factory({ wasmBinary }));
}
...
export class StormArchiveModule {
  ...
  async open(
    ...
    const flags = options.readonly === false
...
export class StormArchive {
  ...
  close(): Uint8Array | undefined {
    ...
    return this.#saveOnClose
      ? this.#module.readVirtualFile("/stormlib/archive.mpq")
      : undefined;
  }
